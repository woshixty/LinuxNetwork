    ...
    while(true)
    {
        ...
        for(int i = 0; i < infds; ++i)
        {
            ...
            if(events[i].data.fd == listenfd)
            {
                ...
                int clientsock = accept(listenfd, (struct sockaddr*)&clientaddr, &clientlen);
            ...
            else
            {
                ...
                else if(events[i].events & EPOLLIN | EPOLLPRI)  // 接收缓冲区有数据可以写
                {
                    ...
                    while(true)
                    {
                        ...
                        int nread = read(events[i].data.fd, buffer, sizeof(buffer));
                        if(nread > 0)
                        {
                ...
                else if(events[i].events & EPOLLOUT)    // 发送缓冲区有空间可以写
                {}
                else
                {
                    // 视为错误 打印socket
