...
int main(int argc, char *argv[])
{
    ...
    while(true)
    {
        ...
        for(auto& event : evs)
        {
            ...
            if(event.events & EPOLLRDHUP)   // 客户端关闭连接 有些系统监测不到 可以使用 EPOLLRDHUP recv() 返回 0
            {
                printf("1 client %d closed\n", event.data.fd);
                close(event.data.fd);
            }
            else if(event.events & EPOLLIN | EPOLLPRI)  // 接收缓冲区有数据可以写
            {
                // 如果发生事件的是listenfd 表示有新的客户端连接上来了
                if(event.data.fd == servsock.fd())
                {
                    // 接受连接请求
                ...
                else
                {
                    ...
                    while(true)
                    {
                        ...
                        int nread = read(event.data.fd, buffer, sizeof(buffer));
                        if(nread > 0)
                        {
                            printf("client %d send data: %s\n", event.data.fd, buffer);
                            send(event.data.fd, buffer, nread, 0);
                        }
            ...
            else if(event.events & EPOLLOUT)    // 发送缓冲区有空间可以写
            {}
            else
            {
                // 视为错误 打印socket
